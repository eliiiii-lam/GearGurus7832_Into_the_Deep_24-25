...
@TeleOp(name = "solo samp", group = "A1")

public class solo_samp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            double triggerInput = gamepad1.right_trigger - gamepad1.left_trigger; // Right trigger increases, left trigger decreases

            if (Math.abs(triggerInput) > 0.1) {
                // Map trigger input (-1 to 1) to servo range (0 to 1)
                double servoPosition = (1 - (triggerInput + 1) / 2);

                // Set the servo position
                inX.setPosition(servoPosition);
            } else {
                // Optional: Keep the servo in a neutral position or maintain its current state
                inX.setPosition(0.5); // Set to neutral if triggers are idle
            }
